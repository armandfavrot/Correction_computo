...
::: response
We added the following sentece in the conclusion : 
:::
...
$$\log\ \lambda_{ijkt} = \alpha_0 + \beta_{0i} + \gamma_{0j}  + (\alpha_1 + \gamma_{1j})\ X_t + u_{ij}$$
...
On ne connait pas le coût d'un essai où l'on mesure Y et le coût d'un essai où l'on mesure Z, on ne peut donc pas savoir quels scénarios considérer pour faire une comparaison sur la base des coûts. Le fait de comparer [50% Y - 0% Z] et [0% Y - 50% Z] montre simplement que la mesure de Y apporte plus de précision dans l'estimation que la mesure de Z (à nombre d'observations égale).

Cependant ce commentaire me fait voir qu'il y a un problème dans l'article : la partie 3.2.3 "Is it better to measure intensity or prevalence in new pest surveys ?". Je pense que pour bien répondre à cette question il faudrait connaitre les coûts des différents scénarios. 
...
::: response 
We made the change, it correspond to the commit "change in notation for scenarios" (dfbaa65).
:::

::: rouge
Ne faudrait-il pas en plus normaliser la notation de la façon suivante : toujours Y en premier et Z en deuxième. Exemple "50% Y - 0% Z", "0% Y - 50% Z", plutôt que "50% Y - 0% Z", "50% Z - 0% Y".
:::
...
::: response_strategy         
Expliquer que ce n'est pas possible car on ne dispose des Z que dans 15 essais sur les 32 (paragraphe 2.1)
:::

::: response
We could not consider all the scenarios defined in Table 3 for the real data because we only have data for Z in 15 out of the 32 trials (see paragraph 2.1, "The number of infested plants (prevalence) was measured as well, but only in 15 trials out of 32"). Therefore, we were unable to consider the scenarios "100% Z - 0% Y," "100% Y - 100% Z," and "100% Z - 50% Y." We also did not consider the scenario "100% W" because W is not measured (only the sum of aphids on the N sugar beets in the sample is reported).
:::
...
::: response_strategy
Expliquer que l'objectif n'était pas de faire une optimisation de design, rajouter une phrase quelque part pour souligner que notre modèle pourra être utiliser pour faire de l'optimisation de design.
:::

::: response
We did not consider more intermediate scenarios because our goal was not to perform an experimental design optimization but simply to demonstrate that it is possible to combine Y and Z and that it is advantageous to do so when only Z is measured in certain trials.

To emphasize one of the benefits of our model being experimental design optimization, we have added the following sentence:
:::

::: rouge
Rajouter une phrase pour dire que notre modèle pourra être utilisé pour faire de l'optimisation de design.
:::
...
::: response_strategy         
Je ne suis pas sur que le nombre de plantes soit un paramètre important. Mon intuition est qu'il est équivalent au nombre de sites. Par exemple je pense que deux sites où on mesure Z sur 20 plantes est équivalent à 4 sites où on mesure Z sur 10 plantes. Le nombre de plantes N pourrait cependant faire partie d'un problème d'optimisation sous contraintes dans lequel on veut par exemple un minimum de site pour couvrir un nombre suffisant de situation environnemental (météo, composition de la terre, présence de pucerons, ...). See [here](#optimisation) .

On pourrait par exemple, étant donné $N_Y$ la taille d'échantillon pour laquelle on mesure l'intensité, chercher $N_Z$ la taille d'échantillon pour laquelle on mesure la prévalence qui soit telle que la précision obtenue avec l'intensité soit égale à la précision obtenue avec la prévalence.

Faire à peu près la même réponse que pour D.2.2 : expliquer que l'objectif n'était pas faire de l'optimisation de design, mais que cela est possible en particulier en considérant N comme un paramètre à optimiser. Rajouter une phrase quelque part pour en parler.
:::

::: response
Yes, the larger N is, the more informative Z becomes, and N could effectively be a part of an experimental design optimization problem. As mentioned in our response to comment D.2., our objective here was not to conduct an optimization of the design. However, we do propose to address this in the conclusion, and we have added the following sentence to it:
:::

::: rouge 
Ajouter la phrase en question
:::
...
::: response_strategy         
Oui, il doit exister un $N_Z$ fonction de $N_Y$, $N_Z = f(N_Y)$, tel que la précision obtenue avec $Y$ soit égale à celle obtenue avec $Z$. Il faudrait ensuite comparer le coût entre mesure de Y sur $N_Y$ betteraves et mesure de $Z$ sur $N_Z$ betteraves pour déterminer la meilleure option. 

Rajouter une phrase dans la conclusion pour souligner ce point.
:::

::: response         
Yes, there must exist an $N_Z$ that is a function of $N_Y$, where $N_Z = f(N_Y)$, such that the precision obtained with $Y$ is equal to that obtained with $Z$. Then, it would be necessary to compare the cost between measuring Y on $N_Y$ sugar beets and measuring $Z$ on $N_Z$ sugar beets to determine the better option.

We have added the following sentence to the conclusion to emphasize this point:
:::
...
::: response_strategy         
Oui, stratégie de réponse : parler du temps de calculs pour justifier le nombre limité de scénarios considérés, en profiter pour citer migale.
:::

::: response
Yes, we have added the following sentence at the end of section 2.2.3 (Material and Method / Model / Simulations):
"These simulations required a significant amount of computation time and were conducted on the INRAE MIGALE server."

And we have removed the following sentence from the note associated with the code on simulations :
"We used the Migale server from INRAE to run these simulations".
:::

::: rouge
Ajouter les phrases, rajouter la citation de Migale 
:::
...
## Problème d'optimisation {#optimisation}
